...
public class NetworkManager : MonoBehaviour
{
    ...
    // InterpolationTick为插值Tick，略慢于ServerTick，差值默认为2 ticks
    ...
    public ushort TicksBetweenPositionUpdates
    {
        ...
        private set
        {
            // 插值Tick与ServerTick差值为_ticksBetweenPositionUpdates，此处默认为2
    ...
    [Space(10)]
    [SerializeField] private ushort ticksDivergenceTolerance = 1;
    ...
    private void Start()
    {
        ...
        // [Riptide特性]
        // Client.A += B 定义在客户端监听A事件，并执行B方法作为响应
        Client.Connected += DidConnect;
        Client.ConnectionFailed += FailedToConnect;
        Client.ClientDisconnected += PlayerLeft;
        Client.Disconnected += DidDisconnect;

        // ServerTick预设为2，避免插值Tick为负数
        ServerTick = 2;
    }
    ...
    private void SetTick(ushort serverTick)
    {
        ...
        if (Math.Abs(ServerTick - serverTick) > ticksDivergenceTolerance)
